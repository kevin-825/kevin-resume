...
#strong[多家芯片公司底层开发经验:] 在多家芯片原厂（包括华为、君正等）的Soc开发软件团队中积累了丰富的芯片底层开发经验，对芯片内部模块比较熟悉(ADIE,BDIE)熟悉从硅前验证到硅后 Bring-up 的全流程，具备扎实的 SoC 架构理解和实际开发能力。

#strong[深入理解 RISC-V 及多种处理器架构:] 熟练使用 C 语言进行底层开发，熟悉 Python、Bash Shell、Matlab\/Simulink 等工具；精通 RISC-V、ARMv7\/v8-M、AArch64 以及 Xtensa DSP 架构；具备丰富的芯片底层开发经验，熟悉 FPGA 验证和 Bring-up；深入理解 RTOS 和 Linux 内核，熟练使用 GNU 工具链和调试工具；善于运用 Generative AI 辅助编程和问题排查。
...
#regular-entry(
  [
    ...
    - #strong[PLIC中断嵌套实现]：无JTAG debug手段的情况下，利用生成式AI（ChatGPT，Copilot和Gemini Pro）完成了复杂的QEMU模拟器模拟riscv CPU和PLIC硬件，完成 CPU 与 PLIC 的软硬件协同仿真，在这个模拟环境下运行FreeRTOS Demo并完成PLIC嵌套中断功能开发，完成开发后 FPGA 上运行验证，成功排查出 SoC 部门提供的子系统中断号偏移的Bug，使能58号中断结果60号中断触发58不触发。
    ...
    - #strong[离职原因：] 主要是加班严重，前期无jtag缺少debug手段，最重要的只有2太FPGA机器8-10个人轮着用，周末基本无休都忙着debug，导致工作效率极低，后期虽然jtag debug环境搭建成功了，但是项目一开始就没有考虑出FGPA芯片image时加入JTAG电路模块，导致很多FGPA芯片image无法使用JTAG debug，TNPU的bug也是加了JTAG电路模块才得以debug成功，严重影响了后续的开发效率和体验，最终导致自愿离职。且公司管理制度比较死板，无偿加班且不能调休，而且早上上班迟到要扣工资。经协商无果，身体难以承受长时间高强度加班，选择自愿离职。
...
#regular-entry(
  [
    #strong[博士视听系统(上海)有限公司 (Bose)] 2024.05 - 2024.06
...
#regular-entry(
  [
    ...
    - #strong[RISC-V PLIC 中断控制器重构与遗留架构缺陷消除]：研究riscv PLIC中断控制器解决了RTC 和gp timer中断无法触发的问题共享中断驱动问题，一步一步排查发现PLIC中断控制器有正常触发中断，但是ISR handler 没有执行，是原来的wq7033的RTC和gp timer存在很多硬件限制性 问题比如存在中断号共享等，7033上的中断处理驱动有很多work around来修复避免的这些问题，这个work wround在更新的7036上完全失效，因为7036消除了中断共享以及其他硬件限制，之前的work aroud会引起7036的bug
