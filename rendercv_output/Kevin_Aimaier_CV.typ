...
#connections(
  [#connection-with-icon("location-dot")[武汉 | 维吾尔族]],
...
#regular-entry(
  [
    ...
    #summary[“添加这一条是为了说明2025.05至今为何没有入职”]
...
#regular-entry(
  [
    ...
    - #strong[提议集成JTAG并软硬联调]：在极度缺乏 FPGA 验证资源（8-10人共用2台，周末无休）且无 JTAG 调试环境的情况下，提议并推进 JTAG 模块集成，并配合 SoC 部门耗时一月有余修复 GDB JTAG连接不稳定断链问题以及JTAG子板问题，成功拉起首套稳定可用的GDB OpenOCD JTAG软硬联调环境，结束了只靠串口打印的“盲人摸象”式无效和FPGA拉线看波形的低效debug窘境，使得debug体验效率有质的提升。

    - #strong[PLIC中断嵌套实现]：无JTAG debug手段的情况下，利用QEMU模拟器模拟riscv CPU和PLIC硬件，完成 riscv CPU 与 PLIC中断控制器的软硬件协同仿真，在这个模拟环境下运行FreeRTOS Demo并完成PLIC嵌套中断功能开发并且在FPGA上运行验证，成功排查出 SoC 部门提供的部分中断号偏移的Bug，使能58号中断结果60号中断触发58不触发，成功实现并验证了PLIC中断嵌套功能，极大提升了系统的中断处理能力和实时响应性能。

    - #strong[Cache\/Spinlock底层适配优化]：研发cache驱动以支持cache flush\/invalidate操作（按地址刷和按cache set index刷）。编写spinlock底层riscv汇编代码实现spinlock功能用以多核间自旋互斥操作。协助同事解决自旋锁死锁问题。

    - #strong[检查修复FreeRTOS底层适配]：检查FreeRTOS底层适配汇编代码，修复底层的是否在中断上下文\/临界区的函数，保证了系统能够在ISR里稳定唤醒bottom half中断延迟处理任务。修复local\_irq\_save和local\_irq\_restore函数，修复了系统无法稳定触发中断的问题，保证了系统的tick中断时序，也保证了临界区中断关闭恢复的正确性。修复FreeRTOS第一个任务Stack初始化函数引起的任务退出中断上下文时发生栈踩踏的 Bug ，提高了系统的稳定性。
...
#regular-entry(
  [
    ...
    - #strong[多核异构 DFx 维测基建与 Xtensa 架构上下文恢复]：负责DFx子系统的适配，这个也就维测子系统。针对7036的复杂异构系统，利用 cpu核所在子系统下的watchdog和pmic全局watchdog、NMI (不可屏蔽中断) 与 PMP (物理内存保护) 等硬件机制，从零构建全局异常捕获框架。精准拦截跨子系统死锁、栈溢出及非法内存访问，生成 Coredump 和ram dump文件，编写脚本把收集到的coredump，ram dump组合成corefile用GDB分析崩溃现场，并用addr2line工具链写脚本解析系统crashlog快速找出系统崩溃时的源码文件行号等，为这三条产品线的顺利量产提供决定性保障。该任务难点在于需要同时设计应用子系统core0，蓝牙子系统core1 和xtensa hifi5 dsp子系统的异常检查。其中，core0\/core1是riscv核，是我熟悉的，适配起来也容易，最难的就是xtensa hifi5 dsp，这是个高并行执行和乱序执行的高性能dsp，需要深刻理解xtensa hifi5 dsp的异常模型，通用寄存器分可见窗口寄存器16个，实际register file是64个，跟中断处理跟arm Coretex-A的GIC一样，分4个常level el0到el3.
...
#regular-entry(
  [
    #strong[MIT 6.828\/6.1810 Operating System Engineering] | MIT OpenCourseWare (麻省理工学院公开课)

    - #link("https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-828-operating-system-engineering-fall-2012/")[MIT Operating System Engineering 官方课程主页]

    - #strong[核心理论基建：] 深入理解操作系统设计与实现的核心概念，包括进程管理、内存管理、文件系统和设备驱动等。

    - #strong[技术收益：] 建立起对现代操作系统架构的全面认识，为深入研究和开发高性能、高可靠性的操作系统提供了坚实的理论基础。
...
#strong[读书:] 喜欢kindle电纸书上阅读，尤其是cpu设计，linux内核开发，操作系统原理，数学等相关的书。
