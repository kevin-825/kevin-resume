...
# 求职意向 Job Intentions
**目标岗位:** 嵌入式软件架构工程师、芯片底层驱动工程师 (BSP)、系统软件开发工程师
...
**SoC 架构与硅后 Bring-up:** 具备多家芯片原厂全流程研发经验；深刻理解复杂 SoC 互联拓扑 (AHB/AXI/APB 总线, Bus Matrix, 时钟/电源树)；精通 BootROM/Bootloader 底层基座确立，深度掌控高速与基础外设控制器 (SFC, SMC, DMA, WDT, SPI/I2C/UART) 工作机理。

**OS 内核与驱动框架:** 具备深厚的 Linux 与 RTOS (FreeRTOS, OpenHarmony) 内核实战功底；精通内核任务调度算法、内存寻址分配、高并发同步原语及 VFS 子系统；熟练掌握 Linux 设备树 (DTS)、字符设备驱动模型、交叉构建 (Buildroot/Kconfig) 及系统引导拉起全流程。

**系统级维测与工程化基建:** 精通 GNU 工具链 (GCC, GDB, nm, objdump) 及 Makefile 构建；具备极其敏锐的极限 Bug 攻坚与现场还原能力 (跨域死锁追踪, 内存踩踏/泄漏分析, 宕机 Coredump 解析)；熟练应用 QEMU 系统级模拟器、Docker 开发环境容器化及 CI/CD (GitLab, Jenkins) 自动化流水线。

**极客素养与综合能力:** 具备无障碍解析全英文硬核技术文献 (DataSheet, Architecture Manual) 与跨国协同交流能力；擅长运用 Generative AI 赋能架构设计与复杂排错；保持极强的技术自驱力，深度涉猎并实践 MIT OCW、Linux Foundation 等顶尖开源架构课程。
...
“添加这一条是为了说明2025.05至今为何没有入职”

- **这一年我在做什么？** 在做自己的个人项目，还学习了linux foundation的很多课程，包括linux kernel internals and development (LFD420)，developing embedded linux device drivers (LFD435)，embedded linux development (LFD450)等课程，这部分可以参考“个人/开源项目 ”和“专业进修”。
...
- **提议集成JTAG并软硬联调**：在极度缺乏 FPGA 验证资源（8-10人共用2台，周末无休）且无 JTAG 调试环境的情况下，提议并推进 JTAG 模块集成，并配合 SoC 部门耗时一月有余修复 GDB JTAG连接不稳定断链问题以及JTAG子板问题，成功拉起首套稳定可用的GDB OpenOCD JTAG软硬联调环境，结束了只靠串口打印的“盲人摸象”式无效和FPGA拉线看波形的低效debug窘境，使得debug体验效率有质的提升。

- **PLIC中断嵌套实现**：无JTAG debug手段的情况下，利用QEMU模拟器模拟riscv CPU和PLIC硬件，完成 riscv CPU 与 PLIC中断控制器的软硬件协同仿真，在这个模拟环境下运行FreeRTOS Demo并完成PLIC嵌套中断功能开发并且在FPGA上运行验证，成功排查出 SoC 部门提供的部分中断号偏移的Bug，使能58号中断结果60号中断触发58不触发，成功实现并验证了PLIC中断嵌套功能，极大提升了系统的中断处理能力和实时响应性能。

- **Cache/Spinlock底层适配优化**：研发cache驱动以支持cache flush/invalidate操作（按地址刷和按cache set index刷）。编写spinlock底层riscv汇编代码实现spinlock功能用以多核间自旋互斥操作。协助同事解决自旋锁死锁问题。

- **检查修复FreeRTOS底层适配**：检查FreeRTOS底层适配汇编代码，修复底层的是否在中断上下文/临界区的函数，保证了系统能够在ISR里稳定唤醒bottom half中断延迟处理任务。修复local_irq_save和local_irq_restore函数，修复了系统无法稳定触发中断的问题，保证了系统的tick中断时序，也保证了临界区中断关闭恢复的正确性。修复FreeRTOS第一个任务Stack初始化函数引起的任务退出中断上下文时发生栈踩踏的 Bug ，提高了系统的稳定性。

- **静态编译windows版OpenOCD并生成容器化环境DockerImage**： 因Windows机器上开虚拟机Linux运行OpenOCD性能损失大难以满足Ghz处理器高速jtag GDB debug需求，编写Dockefile并生成openOCD Windows版本编译环境的DockerImage，用于后续编译更新的OpenOCD源码以支持JTAG debug。OpenOCD源码静态编译Windows版OpenOCD的挑战在于其依赖包非常的多且繁杂，大部分都需要手动编译源码形成静态库才能链接到OpenOCD中，并且编译FLAG需要针对每个依赖包进行细致调整编译FLAG。有了这个容器化编译环境，使得新手搭建编译OpenOPCD源码环境的时间从2-3天缩短到5分钟，极大提升了团队成员更新OpenOCD源码的积极性和效率。
...
## **博士视听系统(上海)有限公司(Bose.com)** 2024.05 - 2024.06
...
- **FreeRTOS 低功耗与 Tickless 休眠唤醒优化**：编写FreeRTOS 单元测试用例去验证多个外设驱动的正确性，包括flash SFC（serial flash controller），UART，WDT。FreeRTOS系统下开启tickless IDLE模式实测多核的低功耗pm模块。修复了uart在Free RTOS进出tickless IDLE低功耗时出现的打印乱码问题（core1 device restore过程中存在还没初始化uart就打印导致uart1 restore异常），以及wdt timer在系统退出tickless IDLE恢复外设状态时无效的问题，通过把wdt的save 和 restore 放到machine mode下作为芯片驱动的一部分而不是注册到外设驱动电源管理pm结构体里，这样就能抓到pm device restore过程中出现的死循环问题，这保证了只要系统上电就保证wdt有效。

- **DDR PSRAM 控制器调优与跨子系统死锁溯源**：负责win bound psram厂商的ddr psram设备驱动修复，开发调试芯片内的SMC（Sysytem memory controller）的驱动，因为当时的psram驱动完全无法用。ddr psram很复杂，从硬件层面psram的SMC控制器设计到复杂的时钟树和电源树的正确配置，以及bus brige 的频率切换以支持psram的严格时序。解决的问题，修改dvfs时钟电源管理代码以保证psram能正常上电并和smc通信做校准，这块涉及到深入学习理解芯片电源管理文档手册。还有一个软件无法独立解决的问题、是跟多个数字部硬件同事联合调试解决的，那就是psram单元测试过程中出现的读写数据出错的问题，这个是完全由数字部同事指点下一步一步定位到是硬件默认启用的clock gate功能在反复换切上下电brige频率时出现了无法消除的时钟毛刺（无法通过调整psram工作频率和校准手段去除）最终关闭clock gate功能得以解决。wq7033芯片上的所有cpu核都用ddr psram内存（编译时分给各个子系统），core0 作为主核心负责管理其他异构子系统，包括给core1， hifi4 dsp核上下电等。在core0应用子系统下跑FreeRTOS 低功耗单元测试，发现了系统进出tickless IDLE后必出现的非常难debug的psram数据内容被写坏且芯片挂死的bug，经过讨论没有人知道是为什么。独立研究并调查发现是因为给dsp核上电的pm电源管理驱动存在bug，给dsp核心上电之前没有stall住dsp核的clock，导致一上电就开始跑，这时候core0 还没完成psram的初始化导致psram工作异常。
...
- **RISC-V PLIC 中断控制器重构与遗留架构缺陷消除**：研究riscv PLIC中断控制器解决了RTC 和gp timer中断无法触发的问题，共享中断驱动问题，一步一步排查发现PLIC中断控制器有正常触发中断，但是ISR handler 没有执行，是原来的wq7033的RTC和gp timer存在很多硬件限制比如存在中断号共享等，7033上的中断处理驱动有很多work around来修复避免的这些问题，这个work wround在更新的7036上完全失效，因为7036消除了中断共享以及其他硬件限制，之前的work aroud会引起7036的bug
...
- **多核异构 DFx 维测基建与 Xtensa 架构上下文恢复**：负责DFx子系统的适配，这个也就维测子系统。针对7036的复杂异构系统，利用 cpu核所在子系统下的watchdog和pmic全局watchdog、NMI (不可屏蔽中断) 与 PMP (物理内存保护) 等硬件机制，从零构建全局异常捕获框架。精准拦截跨子系统死锁、栈溢出及非法内存访问，生成 Coredump 和ram dump文件，编写脚本把收集到的coredump，ram dump组合成corefile用GDB分析崩溃现场，并用addr2line工具链写脚本解析系统crashlog快速找出系统崩溃时的源码文件行号等，为这三条产品线的顺利量产提供决定性保障。该任务难点在于需要同时设计应用子系统core0，蓝牙子系统core1 和xtensa hifi5 dsp子系统的异常检查。其中，core0/core1是riscv核，是我熟悉的，适配起来也容易，最难的就是xtensa hifi5 dsp，这是个高并行执行和乱序执行的高性能dsp，需要深刻理解xtensa hifi5 dsp的异常模型，通用寄存器分可见窗口寄存器16个，实际register file是64个，跟中断处理跟arm Coretex-A的GIC一样，分4个常level el0到el3.
...
- **项目URL：**[https://github.com/kevin-825/QemuRTOS](https://github.com/kevin-825/QemuRTOS)
...
- **项目URL：**[https://github.com/kevin-825/QemuEmbeddedLinux](https://github.com/kevin-825/QemuEmbeddedLinux)
...
- **项目URL：**[https://github.com/kevin-825/EmbeddedDevContainer](https://github.com/kevin-825/EmbeddedDevContainer)
...
- [Linux Foundation 官方课程主页](https://training.linuxfoundation.org/training/linux-kernel-internals-and-development)
...
- [Linux Foundation 官方课程主页](https://training.linuxfoundation.org/training/developing-embedded-linux-device-drivers/)
...
- [Linux Foundation 官方课程主页](https://training.linuxfoundation.org/training/embedded-linux-development/)
...
- [MIT 6.004 官方课程主页](https://ocw.mit.edu/courses/6-004-computation-structures-spring-2017/)
...
**体育运动:** 喜欢排球、羽毛球等团队运动，享受运动带来的乐趣和挑战。

**音乐&乐器:** 喜欢听各种类型的音乐，维吾尔族民乐；收藏乐器，喜欢在闲暇时弹奏和练习音乐。

**读书:** 喜欢kindle电纸书上阅读，尤其是cpu设计，linux内核开发，操作系统原理，数学等相关的书。
